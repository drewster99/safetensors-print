...
# Checked here rather than left to the build, which only objects after the version file
# has been rewritten and the whole suite has run.
if [[ -n "$OVERRIDE_VERSION" ]] &&
   [[ ! "$OVERRIDE_VERSION" =~ ^[0-9]+\.[0-9]+\.[0-9]+((a|b|rc)[0-9]+)?$ ]]; then
  fail "--version ${OVERRIDE_VERSION} is not N.N.N with an optional a/b/rc suffix"
fi
...
# Every path this script deletes is one it generated, all of them under build/release.
# Routed through one guard so a mistyped or empty variable can never turn into an rm -rf
# somewhere else: anything not inside BUILD_ROOT is refused rather than removed.
remove_generated() {
  local target="$1"
  [[ -n "$target" ]] || fail "refusing to remove an empty path"
  # .. is refused outright: a prefix check alone would accept build/release/../../..,
  # which is the root of the disk wearing the right first few characters.
  case "$target" in
    *..*) fail "refusing to remove ${target}: paths with .. are not generated by this script" ;;
    "${BUILD_ROOT}"|"${BUILD_ROOT}"/*) ;;
    *) fail "refusing to remove ${target}: outside ${BUILD_ROOT}" ;;
  esac
  rm -rf "$target"
}
...
# Anything can fail between here and the commit -- a build, a test, a network call. The
# version file must not be left rewritten when it does, or the next run's preflight
# refuses to start on a dirty tree and the reason is a mystery.
VERSION_WRITTEN=0
VERSION_COMMITTED=0
restore_version_if_unfinished() {
  local code=$?
  if [[ "$code" -ne 0 && "$VERSION_WRITTEN" -eq 1 && "$VERSION_COMMITTED" -eq 0 ]]; then
    write_version "$CURRENT_VERSION"
    printf 'restored %s to %s\n' "$VERSION_FILE" "$CURRENT_VERSION" >&2
  fi
}
trap restore_version_if_unfinished EXIT
